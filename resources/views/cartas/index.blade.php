                    ...
                    <tbody>
                        @forelse ($cartas as $carta)
                            <tr>
                                ...
                                <td>
                                    {{ $carta->dFechaRecojo ? \Carbon\Carbon::parse($carta->dFechaRecojo)->format('d/m/Y') : '-' }}
                                </td>
                                <td>{{ $carta->nNroResibo ?? '-' }}</td>
                                <td>{{ $carta->cObservacion ?? '-' }}</td>
                                <td>
                                    <span class="badge bg-{{ $carta->bPresentoSupervision ? 'success' : 'secondary' }}">
                                        {{ $carta->bPresentoSupervision ? 'Sí' : 'No' }}
                                    </span>
                                </td>
                                <td>{{ $carta->empresa->cNombreEmpresa ?? '' }}</td>
                                <td>
                                    <span class="badge bg-{{ $carta->nEstado == 'Aprobado' ? 'success' : ($carta->nEstado == 'Rechazado' ? 'danger' : 'secondary') }}">
                                        {{ $carta->nEstado }}
                                    </span>
                                </td>
                                ...
                                <td>
                                    @if($carta->adjunto)
                                        <a href="{{ asset('storage/'.$carta->adjunto) }}" 
                                           target="_blank"
                                ...
                                <td class="text-center">
                                    <a href="{{ route('cartas.edit', $carta->IdCartaPresentacion) }}" 
                                       class="btn btn-warning btn-sm">
                                        <i class="bi bi-pencil-square"></i> Editar
                                    </a>
                                    <form action="{{ route('cartas.destroy', $carta->IdCartaPresentacion) }}" 
                                          method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
                                                onclick="return confirm('¿Seguro de eliminar este trámite?')" 
